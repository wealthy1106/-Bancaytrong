<?php
  ...
  if(isset($_GET['idloai'])) {
    $idloai = $_GET['idloai'];
  $query = "select * from loaicuthe where idloai = '".$idloai."'";
    try {
        $sth = $pdo->query($query);
        if ($row = $sth->fetch()){
            $tenloai = $row['tenLoai'];
  ...
  } else if(isset($_POST['idloai'])) {
        $idloai = $_POST['idloai'];
        $tenloai = $_POST['tenloai'];
        $maloai = $_POST['maloai'];
        $query = 'UPDATE loaicuthe
        SET tenLoai = ?, Maloai = ?
        WHERE idloai = ?';
        try{
    ...
    <div>
        <div class="row">
            ...
            <div class="col-sm-9">
               ...
               <h1 style="text-align: center;">Sửa Loại Sản Phẩm Cụ Thể</h1>
                <div style="width: 600px;margin: auto;">
                <form action="sualoaisp.php" method="POST">
                    <div class="form-group">
                        <label for="idloai">ID loại</label>
                        <input type="text" readonly class="form-control" id="idloai" name="idloai" value="<?php echo $idloai; ?>">
                    </div>
                    <div class="form-group">
                        <label for="tenloai">Tên Loại</label>
                        <input type="text" class="form-control" id="tenloai" name="tenloai" value="<?php echo $tenloai; ?>" >
                    </div>
                    <div class="form-group">
                        <label for="maloai">Mã Loại</label>
                        <input class="form-control" id="maloai" name="maloai" value="<?php echo $maloai; ?>"></input>
                    </div>
